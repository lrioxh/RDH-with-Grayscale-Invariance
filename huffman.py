...
if __name__ == '__main__':
    ...
    # writeBinImage(img_encode, src_img_path.replace('.bmp', '.bin'))
 
    # 读取编码的文件，得到二进制编码数据字符串
    # img_read_code = readBinImage(src_img_path.replace('.bmp', '.bin'), len(img_encode))
    # 解码二进制编码数据字符串，得到原始图像展开的向量
    # 这是根据哈夫曼树进行解码的方式
    # img_src_val_array = decodeHuffman(img_encode, huffman_root_node)
    # 这是根据编码字典进行解码的方式，更慢一些
    img_src_val_array = decodeHuffmanByDict(img_encode, Huffman_encode_dict)
 
    # 确保解码的数据与原始数据大小一致
    print(len(img_src_val_array),src_img_w * src_img_h)
    ...
    print("压缩率为：{:.6f}".format(len(img_encode)/len(src_img_ravel)/8))